...
const App: React.FC = () => {
  // --- STORAGE PERSISTENCE ---
  useEffect(() => {
    // Request persistent storage to prevent the browser/OS from clearing 
    // localStorage (API Keys) and Cache (Design files) when space is low.
    if (navigator.storage && navigator.storage.persist) {
      navigator.storage.persist().then(granted => {
        if (granted) {
          console.log("Storage persistence granted.");
        } else {
          console.log("Storage persistence not granted. Data might be cleared by OS.");
        }
      });
    }
  }, []);
